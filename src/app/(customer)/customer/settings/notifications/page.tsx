...
import { ArrowLeft, Save, Loader2, Bell, Mail, Smartphone, Calendar, Star, Gift, MessageCircle, Moon } from 'lucide-react';
...
export default function CustomerNotificationSettingsPage() {
  ...
  const NotificationRow = ({ icon, title, desc, checked, onChange }: { icon: React.ReactNode; title: string; desc: string; checked: boolean; onChange: () => void }) => (
      ...
      <div className="max-w-2xl mx-auto px-6 py-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm mb-6">
          ...
          <div className="space-y-1">
            <NotificationRow icon={<Calendar className="w-4 h-4" />} title="Booking Confirmations" desc="Confirmation when you book a service"
              checked={settings.email_booking_confirmation} onChange={() => setSettings(prev => ({ ...prev, email_booking_confirmation: !prev.email_booking_confirmation }))} />
            <NotificationRow icon={<Bell className="w-4 h-4" />} title="Booking Reminders" desc="Reminders before your appointments"
            ...
            <NotificationRow icon={<MessageCircle className="w-4 h-4" />} title="Messages" desc="When you receive a message"
              checked={settings.email_new_message} onChange={() => setSettings(prev => ({ ...prev, email_new_message: !prev.email_new_message }))} />
            <NotificationRow icon={<Gift className="w-4 h-4" />} title="Promotions & Offers" desc="Special deals and discounts"
              checked={settings.email_promotions} onChange={() => setSettings(prev => ({ ...prev, email_promotions: !prev.email_promotions }))} />
            <NotificationRow icon={<Star className="w-4 h-4" />} title="Review Reminders" desc="Reminders to review completed services"
              checked={settings.email_review_reminder} onChange={() => setSettings(prev => ({ ...prev, email_review_reminder: !prev.email_review_reminder }))} />
          </div>
        </div>
        {/* Push Notifications */}
        <div className="bg-white rounded-2xl p-6 shadow-sm mb-6">
          ...
          <div className="space-y-1">
            <NotificationRow icon={<Calendar className="w-4 h-4" />} title="Booking Confirmations" desc="Instant alerts for bookings"
              checked={settings.push_booking_confirmation} onChange={() => setSettings(prev => ({ ...prev, push_booking_confirmation: !prev.push_booking_confirmation }))} />
            ...
            <NotificationRow icon={<Calendar className="w-4 h-4" />} title="Cancellations" desc="Alerts for cancellations"
            ...
            <NotificationRow icon={<MessageCircle className="w-4 h-4" />} title="Messages" desc="Instant message alerts"
              checked={settings.push_new_message} onChange={() => setSettings(prev => ({ ...prev, push_new_message: !prev.push_new_message }))} />
            <NotificationRow icon={<Gift className="w-4 h-4" />} title="Promotions" desc="Special offers and deals"
              checked={settings.push_promotions} onChange={() => setSettings(prev => ({ ...prev, push_promotions: !prev.push_promotions }))} />
          </div>
        </div>
